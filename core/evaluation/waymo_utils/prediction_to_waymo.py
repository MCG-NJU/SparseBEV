...
class Prediction2Waymo(object):
    ...
    def convert_one(self, res_idx: int):
        ...
        if len(self.results[res_idx]['labels_3d']) > 0:
            objects = self.parse_objects_from_origin(
                ...
                # NOTE modify str to int
                int(self.results[res_idx]['timestamp']))
        else:
    ...
    def convert(self):
        ...
        # len(self): 39987  chunksize must be more than 1 to speed up
        objects_list = mmcv.track_parallel_progress(self.convert_one, range(len(self)), nproc=48, chunksize=200)
        print('\nFinished ...')
